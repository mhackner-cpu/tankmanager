...
export const categoryFieldSchema = z.object({
  key: z
    ...
    .regex(
      /^[a-z_]+$/,
      'Feldschlüssel darf nur Kleinbuchstaben und _ enthalten',
    ),
  label: z.string().min(1, 'Feldbezeichnung ist erforderlich').max(100),
  type: z.enum(['TEXT', 'NUMBER', 'DATE', 'SELECT', 'TEXTAREA']),
  required: z.boolean().default(false),
  optionsJson: z.string().optional(), // JSON-String für SELECT-Optionen
...
export const createCategorySchema = z.object({
  ...
  inventoryPrefix: z
    ...
    .regex(
      /^[A-Z0-9]+$/,
      'Präfix darf nur Großbuchstaben und Zahlen enthalten',
    ),
